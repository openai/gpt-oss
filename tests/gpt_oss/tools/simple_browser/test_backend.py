...
from gpt_oss.tools.simple_browser.backend import YouComBackend, ParallelBackend
...
def mock_os_environ_get(name: str, default: Any = "test_api_key"):
    assert name in ["YDC_API_KEY", "PARALLEL_API_KEY"]
    return default
...
@pytest.mark.asyncio
@mock.patch("aiohttp.ClientSession.post")
async def test_parallel_backend_search(mock_session_post):
    backend = ParallelBackend(source="web")
    api_response = {
        "search_id": "search_test123",
        "results": [
            {"title": "Result 1", "url": "https://www.example.com/1", "excerpts": ["Excerpt 1a", "Excerpt 1b"]},
            {"title": "Result 2", "url": "https://www.example.com/2", "excerpts": ["Excerpt 2a"]},
            {"title": "Result 3", "url": "https://www.example.com/3", "excerpts": []},
        ]
    }
    with mock.patch("os.environ.get", wraps=mock_os_environ_get):
...
@pytest.mark.asyncio
@mock.patch("aiohttp.ClientSession.post")
async def test_parallel_backend_fetch(mock_session_post):
    backend = ParallelBackend(source="web")
    api_response = {
        "extract_id": "extract_test123",
        "results": [
            {"title": "Page Title", "url": "https://www.example.com/page", "full_content": "This is the full page content", "excerpts": None, "publish_date": None},
        ],
        "errors": []
    }
    with mock.patch("os.environ.get", wraps=mock_os_environ_get):
        ...
        assert "This is the full page content" in result.text
