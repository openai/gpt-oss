...
def main(args):
    ...
    with open(options.dst, "wb") as dst:
        with safe_open(os.path.join(options.src, "model.safetensors"), framework="pt", device="cpu") as src:
            ...
            yarn_low = (
                head_dim / 2 * math.log(initial_context_length / (rope_ntk_beta * 2 * math.pi)) / math.log(rope_theta)
            )
            yarn_high = (
                head_dim / 2 * math.log(initial_context_length / (rope_ntk_alpha * 2 * math.pi)) / math.log(rope_theta)
            )
